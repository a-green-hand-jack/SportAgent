...
The agent-native repository root is four levels above this file. This directory remains the runnable
SportAgent product root and contains its `pyproject.toml`, `uv.lock`, source, data, and tests.
...
cd SportAgent/lab/code/imported/SportAgent
uv sync
```
...
### 6. Incremental Context Accumulation (Multi-Step LLM Agents)

**Problem**: Most LLM providers have large _input_ limits (128k–1M tokens) but small _output_ limits (8k–32k tokens). Generating a long structured artifact (e.g., 7-day meal plan) in a single call hits the output limit and truncates. Splitting into parallel/sequential batches without shared context causes **repetition and inconsistency** between segments.

**Solution**: Generate the artifact **one logical unit at a time** (e.g., one day), and inject all previously generated units as context into each subsequent call.

```
Call 1: [system] + [user ctx] + [data] → generate unit 1
Call 2: [system] + [user ctx] + [data] + [unit 1 JSON] → generate unit 2
Call 3: [system] + [user ctx] + [data] + [unit 1+2 JSON] → generate unit 3
```

**Why this works**:

- **Output per call**: ~1,500 tokens (1 unit) — fits any model's output limit
- **Input grows incrementally**: ~13k baseline + 1.5k × N, peaks at ~22k tokens — well within any provider's input window
- **Model sees all history**: natural diversity enforcement (no repetition), consistent style, no context discontinuity

**When to use this pattern**:

- Output artifact is **longer than one LLM response** can produce
- Units have **interdependencies** (each day's meals should differ from previous days, each chapter should not repeat themes, etc.)
- Quality requires **global coherence** across all units

**Implementation pattern** (see `cooking/agent.py: _generate_day`):

```python
generated: list[Unit] = []
for unit_id in all_units:
    history_json = json.dumps([u.model_dump() for u in generated], ...) if generated else None
    unit = call_llm(unit_id=unit_id, already_generated=history_json)
    generated.append(unit)
```

**Prompt section to add** (see `cooking/prompt.py`):
...
**Trade-off**: N × API calls instead of 1. Each call is faster (smaller output), so wall-clock time is roughly similar. This is generally acceptable for offline/async generation tasks.

**Applied in this project**: `CookingAgent._generate_day()` — generates each of 7 days separately, injecting prior days as JSON context. This fixed the "AB template loop" issue where the model would repeat the same recipes across days.
