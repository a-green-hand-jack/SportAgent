...
SportAgent 已采用 agent-native 仓库结构。产品代码、数据、测试和 Python 项目元数据位于
`lab/code/imported/SportAgent/`；本页中的产品命令均从该目录运行。仓库根目录保留 agent
治理、memory、计划、交付物和验证工具。
...
cd SportAgent
cd lab/code/imported/SportAgent
...
```
SportAgent/
├── AGENTS.md             # Agent 入口与仓库级规则
├── PROJECT.md            # 项目范围、协作和运行约定
├── memory/               # 当前状态与长期协作记忆
├── human/                # 决策、评审与迁移冲突留档
├── scripts/              # Agent-native 治理与验证脚本
└── lab/code/imported/SportAgent/
    ├── src/fitness_agent/
